...
```text
[Diagram converted to Gliffy — see .gliffy file]
graph TB
...
```yaml
# TeamCity (Kotlin DSL pipelines) Pipeline for Pilet
trigger:
  branches:
    include:
      - main
...
- **Build Tools**: Vite 7 (Rolldown bundler + SWC) 5, ESBuild
- **Cloud Platform**: Azure (Static Web Apps, Front Door, Blob Storage)
- **Monitoring**: Application Insights, Custom Telemetry
- **CI/CD**: TeamCity (Kotlin DSL pipelines) Pipelines
- **Testing**: Vitest + React Testing Library, React Testing Library, Playwright
